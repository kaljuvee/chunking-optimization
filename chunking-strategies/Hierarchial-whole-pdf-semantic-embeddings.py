#!/usr/bin/env python3
"""
Hierarchical Chunker (Whole PDF) with Semantic Similarity Filtering
==================================================================

Performs multi-level hierarchical chunking of the entire PDF text
...
import httpx
import tiktoken
from azure_hierarchical import HierarchicalChunker  
...
def semantic_filter(chunks: List[Dict], threshold: float = 0.7) -> List[Dict]:
    ...
    filtered = []
    prev_chunk = chunks[0]
    prev_chunk["embedding"] = get_embedding(prev_chunk['text'])
    prev_emb = prev_chunk["embedding"]

    for curr_chunk in chunks[1:]:
        curr_chunk["embedding"] = get_embedding(curr_chunk['text'])
        curr_emb = curr_chunk["embedding"]
...
def main():
    parser = argparse.ArgumentParser(description="Whole-PDF hierarchical chunking + semantic similarity filter")
    ...
    hierarchy = chunker.chunk_text(full_text)
    flat_chunks = chunker.get_flat_chunks(hierarchy)

    logger.info(f"🧩 Total raw chunks before filtering: {len(flat_chunks)}")
    logger.info(f"🔍 Filtering {len(flat_chunks)} chunks using cosine similarity ≥ {0.7}")
    filtered_chunks = semantic_filter(flat_chunks, threshold=0.7)

    with open(args.output, 'w', encoding='utf-8') as f:
        json.dump(filtered_chunks, f, ensure_ascii=False, indent=2)

    logger.info(f"✅ Saved {len(filtered_chunks)} semantically filtered full-document chunks to {args.output}")
